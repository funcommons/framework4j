...
| 10104 | 请求方法不支持 | 405 路由层拦截（如 POST 调到 GET 端点） | 提示方法不允许 |
| 10105 | 媒体类型不支持 | 415 路由层拦截（Content-Type 不匹配） | 检查请求头 |
| 10106 | 业务规则校验失败 | 业务层主动判定（如余额不足、状态不符） | 显示具体规则提示 |

> 📝 **v1.7 修订**：10104/10105 重新登记为路由层错误码（与 ApiCode 对齐），不再是"永久弃用"。10106 业务规则校验失败作为业务层通用兜底。
...
| 10207 | 令牌格式错误 | JWT 三段式破坏、Base64 解码失败 | 清除本地凭证，跳登录页 |
| 10208 | 令牌已注销 | access token jti 在撤销集合中（主动登出/踢下线） | 跳登录页 |
| 10210 | 刷新令牌已过期 | refresh token family 不存在或 TTL 到期 | 重新登录 |
| 10211 | 刷新令牌无效或已使用 | refresh type 错 / 签名错 / family 毒丸触发 / 已被轮转 | 重新登录（疑似重用攻击） |
| 10212 | 刷新令牌轮转次数超限 | family generation 达到 maxRotations 上限 | 重新登录 |
...
| 10503 | 文件上传失败 | IO 异常/存储服务异常（v1.7：原 10600 迁入） | 提示重试 |
...
> v1.7 修订：原 10600-10603 文件上传错误码已整合到 §7.9（10503-10506），106xx 段释放为业务自定义段。

| 错误码 | 描述 | 适用场景 | 前端处理建议 |
|---|---|---|---|
| 106xx | 业务自定义 | 通用错误码（10xxx ~ 105xx）无法精确表达的业务规则（如余额不足、风控、农时冲突） | 显示具体业务提示 |

> 📝 **登记规范**：106xx 段由各业务线按需登记认领，本规范不预定义具体值。登记时需注明：码值、含义、触发条件、处理建议、认领模块。详见 §7.10.1。
