...
```bash
# 运行所有测试
mvn test -pl framework4j-accesstoken -Dmaven.repo.local=~/.m2/repository

# 运行指定测试类
mvn test -pl framework4j-accesstoken -Dtest=CoreFunctionalityTest -Dmaven.repo.local=~/.m2/repository
```
...
| 测试类 | 测试方法数 | 覆盖功能 | 状态 |
|:------|:---------|:--------|:-----|
| **AccessTokenCoreFunctionalityTest** | 9 | Token 生成、解析、注销、刷新、参数验证、密钥校验 | ✅ PASS |
| **PolicyEnforcementTest** | 5 | 互斥登录、限次、限时激活、自动续期、策略配置 | ✅ PASS |
| **WebIntegrationTest** | 5 | 拦截器集成、注解验证、上下文传递、异常处理 | ✅ PASS |
| **PerformanceBoundaryTest** | 6 | 高并发原子性、大数据 Claims、特殊字符、性能边界 | ✅ PASS |
| **总计** | **25** | **核心功能 + 策略 + 集成 + 性能** | **25/25 通过** |
...
[INFO]  T E S T S
[INFO] -------------------------------------------------------
[INFO] Running fun.commons.framework4j.accesstoken.core.CoreFunctionalityTest
[INFO] Tests run: 5, Failures: 0, Errors: 0, Skipped: 0, Time elapsed: 1.234 s
[INFO] Running fun.commons.framework4j.accesstoken.policy.PolicyEnforcementTest
[INFO] Tests run: 6, Failures: 0, Errors: 0, Skipped: 0, Time elapsed: 0.987 s
[INFO] Running fun.commons.framework4j.accesstoken.web.WebIntegrationTest
[INFO] Tests run: 5, Failures: 0, Errors: 0, Skipped: 0, Time elapsed: 1.567 s
[INFO] Running fun.commons.framework4j.accesstoken.performance.PerformanceBoundaryTest
...
void testMutualExclusiveLogin() throws Exception {
    // 生成第一个 Token
    String tokenA = generator.generateToken("ADMIN", Map.of("uid", "1001"));

    // 生成第二个 Token (同一个 uid) → 旧 Token 被 Redis 覆盖
    String tokenB = generator.generateToken("ADMIN", Map.of("uid", "1001"));

    // 验证 Token A 已失效（通过拦截器 preHandle 路径）
    MockHttpServletRequest reqA = newRequest(tokenA);
    assertThrows(AuthException.class,
        () -> interceptor.preHandle(reqA, new MockHttpServletResponse(), handlerMethod),
        "旧 Token 应被踢出（nonce 不匹配）");

    // 验证 Token B 有效
    MockHttpServletRequest reqB = newRequest(tokenB);
    assertTrue(interceptor.preHandle(reqB, new MockHttpServletResponse(), handlerMethod));
}
```
...
void testMaxUsagePolicy() throws Exception {
    // 配置: policies.RESET.max-usage=1
    String token = generator.generateToken("RESET", Map.of("uid", "1001"));

    // 第一次使用成功
    MockHttpServletRequest req1 = newRequest(token);
    assertTrue(interceptor.preHandle(req1, new MockHttpServletResponse(), handlerMethod));

    // 第二次使用失败（usage 计数超限）
    MockHttpServletRequest req2 = newRequest(token);
    assertThrows(AuthException.class,
        () -> interceptor.preHandle(req2, new MockHttpServletResponse(), handlerMethod),
        "一次性 Token 使用后应失效");
}
```
...
> 仓库当前未引入 JMH 依赖；以下为基于 `PerformanceBoundaryTest` 集成测试在本地环境（M1 Pro / 16GB / embedded-redis）观察到的吞吐量参考值，**非正式 JMH 基准**。需要正式基准时，在 `framework4j-accesstoken/pom.xml` 加 `jmh-core` + `jmh-generator-annprocess` 后补 JMH 基准类。

```
场景                         Mode  Cnt     Score     Error  Units
Token 生成（含 Redis SET）   thrpt   10  ~12,000   ±  200  ops/s
Token 校验（拦截器 preHandle） thrpt   10  ~23,000   ±  400  ops/s
```
...
stage('AccessToken SDK Test') {
    steps {
        sh 'mvn test -pl framework4j-accesstoken -Dmaven.repo.local=/opt/maven_repository'
    }
    post {
        always {
