...
- [使用指南](#使用指南)
  ...
  - [验证 Token](#2-验证-token)
  - [Token 注销](#3-token-注销)
  - [Token 刷新（Refresh Token 机制）](#4-token-刷新refresh-token-机制)
...
framework4j:
  access-token:
    enabled: true
    secret-key: your-secret-key-change-me-with-at-least-32-characters  # 必填，JWT 签名密钥（≥32 字符）
    hash-salt: must-be-non-empty  # 必填，Redis Key 加密盐值
    expire-time: 7200  # 全局默认过期时间（秒）
    redis-name: stringRedisTemplate  # 可选，指定 Redis Bean 名称
...
import fun.commons.framework4j.api.ApiResponse;  // 来自 framework4j-core
...
public class UserController {
    ...
    public ApiResponse<UserProfile> getProfile() {
        // 从上下文获取 Token 信息
        Long uid = TokenContext.<Long>getClaim("uid");
        String username = TokenContext.<String>getClaim("username");
...
| `secretKey` | String | 是 | - | JWT 签名密钥（≥32 字符 / 256 位），建议复杂且定期更换 |
| `hashSalt` | String | 是 | - | Redis Key 哈希盐值（必填非空） |
| `expireTime` | long | 否 | `7200` | 全局默认过期时间（秒，access_token ≤ 2h） |
...
policies:
  multiDevice:
    ...
    expireTime: 7200  # 2小时（access_token 上限）
    autoRenew: true
```
...
public class OrderController {
    ...
    public ApiResponse<Long> createOrder(@RequestBody CreateOrderDTO dto) {
        ...
        Long uid = TokenContext.<Long>getClaim("uid");
        String role = TokenContext.<String>getClaim("role");
...
> **新机制（推荐）**：使用 `generateTokenPair` / `refreshAccessToken` 实现标准 access+refresh 双令牌方案。
> 旧版"重新生成 access"的方式不支持一次性使用 / 轮转 / 家族撤销，建议迁移。

#### 4.1 登录签发 Access + Refresh Pair
...
public class AuthController {
    ...
    /** 登录成功 → 返回 access + refresh + familyId */
    @PostMapping("/login")
    public ApiResponse<TokenPair> login(@RequestBody Map<String, Object> claims) {
        // claims 至少含业务 type 字段（如 "WEB" / "ADMIN"）
        TokenPair pair = tokenGenerator.generateTokenPair(claims);
        return ApiResponse.success(pair);
    }

    /**
     * 用 refresh 换新 pair（一次性使用，原子轮转）
     * 注解 type="refresh" 路由到 refresh 校验分支
     */
    @PostMapping("/refresh")
    @RequiresToken(value = "WEB", type = "refresh")
    public ApiResponse<TokenPair> refresh(@RequestHeader("Authorization") String authHeader) {
        // refresh token 在 Authorization 头（拦截器只校验，不传给 controller）
        String refreshToken = authHeader.substring("Bearer ".length());
        TokenPair newPair = tokenGenerator.refreshAccessToken(refreshToken);
        return ApiResponse.success(newPair);
    }

    /** 主动登出 → 撤销当前 access token */
    @PostMapping("/logout")
    @RequiresToken(value = "WEB")
    public ApiResponse<Void> logout(@RequestHeader("Authorization") String authHeader) {
        String accessToken = authHeader.substring("Bearer ".length());
        tokenGenerator.revokeToken(accessToken);
        return ApiResponse.success(null);
    }

    /**
     * 强制下线（撤销整个 family）—— 适合"踢人" / "改密后失效所有设备" 场景
     * 消费者应用需自行记录 familyId 关联（access token 当前不携带 familyId claim）
     */
    @PostMapping("/force-logout")
    public ApiResponse<Void> forceLogout(@RequestParam String familyId) {
        tokenGenerator.revokeFamily(familyId);
        return ApiResponse.success(null);
    }
}
```
...
```json
{
  ...
  "data": {
    "accessToken": "eyJ...",
    "refreshToken": "eyJ...",
    "familyId": "15348118-0306-4e5d-a7b7-3f45ee11333d",
...
| 特性 | 说明 |
|---|---|
| **一次性使用** | refresh token jti 每次轮转后被标记 consumed=true，再次使用 → 10211 |
| **重用检测** | 旧 refresh 被复用 → 自动撤销整个 family（含所有 access token） |
| **家族机制** | 一族 token 共享 familyId，吊销时一次性全部失效 |
| **轮转上限** | 默认 20 次（可在 Policy 中配置 `maxRotations`），达到后强制重新登录 |
| **TTL 配置** | access 默认 2h，refresh 默认 30d（Policy.refreshExpireTime） |
| **路由隔离** | `@RequiresToken(type="refresh")` 走 refresh 校验路径，access token 无法越权调用 |
...
framework4j:
  access-token:
    policies:
      WEB:
        key: [uid]                  # 互斥登录
        expireTime: 7200            # access 2h
        refreshExpireTime: 2592000  # refresh 30d
        maxRotations: 20            # 家族轮转上限
        autoRenew: true
```
...
| code | 触发场景 | 处理建议 |
|---|---|---|
| 10210 | refresh 已过期或 family 不存在 | 重新登录 |
| 10211 | refresh 无效 / type 错 / 签名错 / 已被使用（重用） | 重新登录 |
| 10212 | family 轮转次数超 maxRotations | 重新登录 |
...
public class BusinessService {
    ...
    public void processOrder() {
        ...
        Long uid = TokenContext.<Long>getClaim("uid");
        String username = TokenContext.<String>getClaim("username");
        List<String> permissions = TokenContext.<List>getClaim("permissions");
...
policies:
  coupon:
    key: [couponCode]  # 优惠券码
    expireTime: 7200  # 2小时（access_token 上限，coupon 可走 refresh 流程）
    maxUsage: 1  # 仅能使用1次
```
...
framework4j:
  access-token:
    secret-key: ${TOKEN_SECRET}  # 从环境变量读取
    hash-salt: ${TOKEN_SALT:default-salt}  # 支持默认值
```
...
import fun.commons.framework4j.accesstoken.context.TokenContext;
import fun.commons.framework4j.api.ApiResponse;  // 来自 framework4j-core
import fun.commons.framework4j.api.ApiException;  // 来自 framework4j-core
...
info("Token 解析成功: {}",payload);
}catch(
AuthException e){
        log.

error("Token 解析失败: {}",e.getMessage());
        }
```
...
| 10208 | Token 已注销 | 200 | 重新登录 |
| 10210 | Refresh 已过期或 family 不存在 | 200 | 重新登录 |
| 10211 | Refresh 无效 / 已被使用（重用检测）| 200 | 重新登录 |
| 10212 | Refresh 轮转次数超限 | 200 | 重新登录 |
...
<configuration>
    <!-- AccessToken 相关日志 -->
    <logger name="fun.commons.framework4j.accesstoken" level="INFO"/>

    <!-- 开启详细日志（生产环境建议设为 WARN） -->
    <logger name="fun.commons.framework4j.accesstoken.interceptor.TokenInterceptor" level="DEBUG"/>
    ...
    <logger name="fun.commons.framework4j.accesstoken" level="INFO" additivity="false">
