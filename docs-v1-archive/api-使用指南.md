...
| 类名                         | 包路径                              | 作用                           |
|------------------------------|-------------------------------------|-------------------------------|
| `ApiResponse<T>`             | `fun.commons.framework4j.api`             | 统一响应体封装                 |
| `ApiCode`                    | `fun.commons.framework4j.api`             | 错误码枚举                     |
| `ApiException`               | `fun.commons.framework4j.api`             | 业务异常类                     |
| `ApiAssert`                  | `fun.commons.framework4j.api`             | 业务断言工具类                 |
| `GlobalExceptionHandler`     | `fun.commons.framework4j.api`             | 全局异常处理器                 |
| `TraceConfig`                | `fun.commons.framework4j.api`             | 全链路追踪配置                 |
| `TraceContext`               | `fun.commons.framework4j.api`             | Trace ID 上下文工具            |
...
import fun.commons.framework4j.api.ApiException;
import lombok.RequiredArgsConstructor;
...
| 10207 | `TOKEN_INVALID_FORMAT` | 令牌格式错误            |
| 10208 | `TOKEN_REVOKED`      | 令牌已注销                |
| 10210 | `REFRESH_EXPIRED`    | 刷新令牌已过期            |
| 10211 | `REFRESH_INVALID`    | 刷新令牌无效或已使用      |
| 10212 | `REFRESH_ROTATION_EXCEEDED` | 刷新令牌轮转次数超限 |
...
#### 105xx: 流量控制与文件上传类（v1.7：文件上传从 106xx 迁入）
...
| 10503 | `UPLOAD_FAILED`      | 文件上传失败（原 10600）   |
| 10504 | `FILE_TYPE_ERROR`    | 文件类型不支持（原 10601） |
| 10505 | `FILE_SIZE_EXCEED`   | 文件体积过大（原 10602）   |
| 10506 | `FILE_EMPTY`         | 文件内容为空（原 10603）   |
...
| 错误码 | 枚举名称 | 说明 |
|-------|---------|------|
| 106xx | （业务登记） | 通用错误码无法精确表达的业务规则（如余额不足、风控、农时冲突）。各业务线按需登记认领，本枚举不预定义。 |
...
| 错误码 | 枚举名称           | 说明 |
|-------|-------------------|------|
| 10700 | `PARTIAL_SUCCESS` | 部分成功（同步批量 N 条中 M 条失败，`data` 非 null 含 `failures`） |
...
- [framework4j-sdk 用户指南](../framework4j-sdk%20用户指南.md)
- [Spring Boot 3 + Micrometer 全链路 SQL 追踪方案](../framework4j-datasource/Spring Boot 3 + Micrometer 全链路 SQL 追踪方案.md)

---

**文档版本:** v1.3
**最后更新:** 2025-12-03
**适用版本:** framework4j-core 1.0.0+
**作者:** LDX2T 架构团队
