...
多 Datasource 数据源注入器是 Framework4j SDK 的核心组件之一,为企业应用提供多数据库数据源管理和注入能力。该组件解决了微服务架构中应用需要连接多个数据库的场景需求。
...
```java
// 添加数据源（实际 API：addDataSource(String name, DataSourceProperties config)）
DataSourceProperties config = new DataSourceProperties();
config.setUrl("jdbc:mysql://db-host.com:3306/tenant_db");
config.setUsername("tenant_user");
config.setPassword("tenant_pass");
config.setDriverClassName("com.mysql.cj.jdbc.Driver");
dataSourceManager.addDataSource("tenant-" + tenantId, config);
...
- `framework4j.datasource.count`:数据源总数
- `framework4j.datasource.connection.active`:活跃连接数
- `framework4j.datasource.connection.idle`:空闲连接数
- `framework4j.datasource.connection.create`:创建连接次数
- `framework4j.datasource.connection.destroy`:销毁连接次数
- `framework4j.datasource.health.status`:健康检查状态
...
2. 确认 `framework4j.datasource.enabled=true`
3. 检查数据源名称是否唯一
...
2. `framework4j.datasource.*` (我们的 SDK 配置)
...
1. **检查是否同时配置了 `spring.datasource.*` 和 `framework4j.datasource.*`**
