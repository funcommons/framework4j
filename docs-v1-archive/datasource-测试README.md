...
# 运行单个测试类
mvn test -Dtest=DataSourceOnAnnotationTest -Dmaven.repo.local=~/.m2/repository

# 运行功能测试
mvn test -Dtest=**/functional/*Test -Dmaven.repo.local=~/.m2/repository

# 运行单元测试
mvn test -Dtest=**/unit/*Test -Dmaven.repo.local=~/.m2/repository
```
...
1. 在 `src/test/java//commons/datasource/` 下创建测试类
