...
>
> 📖 **配套速查文档**：[`Java开发时间处理规范.md`](./Java开发时间处理规范.md)（458 行，5 种格式速查 + 错误案例 + 检查清单，适合新人上手 / 代码评审）
> 📖 **本文件**：详尽方案（含 5 种格式完整决策树、PostgreSQL/Java/前端完整实现、动态时间过滤器、DateTimeUtils 工具类源码），适合需要深度实现的场景
