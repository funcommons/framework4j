...
/**
 * Redis 数据源注入处理器（内嵌于 MultiRedisAutoConfiguration）
 *
 ...
 */
@Slf4j
public static class RedisOnBeanPostProcessor implements BeanPostProcessor, EnvironmentAware {
...
> 实际实现中，`RedisOnBeanPostProcessor` 是 `MultiRedisAutoConfiguration` 的**内嵌静态类**（非独立 `@Component`），由 `MultiRedisAutoConfiguration` 通过 `@Bean` 注册。本节展示其精简结构。
...
public class MultiRedisAutoConfiguration {
    ...
    /**
     * RedisOn 注解处理器（内嵌静态类）
     * 处理 @RedisOn 注解，自动注入指定数据源的 RedisTemplate / RedissonClient
     *
     * @since 1.0.0
     */
    public static class RedisOnBeanPostProcessor implements BeanPostProcessor, EnvironmentAware {
        // ... 实现 @RedisOn 类级 / 字段级注入逻辑
    }
}
```
