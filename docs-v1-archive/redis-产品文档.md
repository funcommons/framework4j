...
多 Redis 数据源注入器是 Framework4j SDK 的核心组件之一,为企业应用提供多 Redis 实例管理和注入能力。该组件解决了微服务架构中应用需要连接多个 Redis 集群的场景需求。
...
> 实际 API：`MultiRedisManager` 不提供"传入 config 自动构建"的高级方法。运行时动态接入需自行构造 `RedisTemplate` / `RedissonClient` 后调用 `registerRedisTemplate(String, RedisTemplate)` / `registerRedissonClient(String, RedissonClient)`。批量配置仍走 §3 配置文件方式。

```java
// 动态注册已构造好的 RedisTemplate
RedisTemplate<String, Object> template = new RedisTemplate<>();
// ... 配置 template 的 connectionFactory / serializer / keySerializer 等
redisManager.registerRedisTemplate("tenant-" + tenantId, template);

// 注册对应的 RedissonClient
RedissonClient client = Redisson.create(redissonConfig);
redisManager.registerRedissonClient("tenant-" + tenantId, client);

// 注：MultiRedisManager 当前未提供 removeDataSource 方法。
// 多租户场景建议通过重启或重新初始化 MultiRedisManager 实例回收资源。
```
...
- `framework4j.redis.datasource.count`:数据源总数
- `framework4j.redis.connection.active`:活跃连接数
- `framework4j.redis.connection.idle`:空闲连接数
- `framework4j.redis.operation.success`:操作成功次数
- `framework4j.redis.operation.failure`:操作失败次数
- `framework4j.redis.health.status`:健康检查状态
...
2. 确认 `framework4j.redis.enabled=true`
3. 检查数据源名称是否唯一
