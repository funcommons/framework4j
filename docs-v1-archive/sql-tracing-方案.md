...
flowchart TD
    ...
    I --> J[读取配置<br/>framework4j.datasource<br/>.datasources.{name}.sql-tracing]
    J --> K{配置是否存在?}
    K -->|否| L[跳过该数据源]
...
本方案已集成在 `framework4j-datasource` 模块中,使用时只需引入:
...
**文件位置:** `fun.commons.framework4j.datasource.tracing.TraceIdDruidFilter`
...
**文件位置:** `fun.commons.framework4j.datasource.tracing.DefaultTraceIdProvider`
...
**文件位置:** `fun.commons.framework4j.datasource.tracing.SqlTracingAutoConfiguration`
...
**文件位置:** `fun.commons.framework4j.datasource.tracing.SqlTracingProperties`
...
logging:
  level:
    fun.commons.framework4j.datasource.tracing: DEBUG
...
**文档版本:** v2.0
**最后更新:** 2025-12-03
**适用版本:** framework4j-datasource 1.0.0+
**作者:** LDX2T 架构团队
