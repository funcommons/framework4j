...
class ApiResponseAdvancedTest {
    ...
    void testNullValueSerialization() {
        ...
        String jsonWithoutNulls = JSON.toJSONString(response);
        assertFalse(jsonWithoutNulls.contains("\"data\":null"));

        // 使用 WriteNulls 特性，null字段应该被序列化
        String jsonWithNulls = JSON.toJSONString(response, JSONWriter.Feature.WriteNulls);
        assertTrue(jsonWithNulls.contains("\"data\":null"));
    }

    @Test
    @DisplayName("测试error字段序列化行为")
    void testErrorFieldSerialization() {
        // error为null时应该被序列化（使用WriteNulls）
        ApiResponse<Void> response1 = ApiResponse.success();
        String json1 = JSON.toJSONString(response1, JSONWriter.Feature.WriteNulls);
        ...
        String json2 = JSON.toJSONString(response2);
    ...
    void testSerializationConsistency() {
        ...
        // 序列化
        String json = JSON.toJSONString(original, JSONWriter.Feature.WriteNulls);

        // 反序列化
        ApiResponse<?> deserialized = JSON.parseObject(json, ApiResponse.class);
    ...
    void testMultiLevelErrorDetails() {
