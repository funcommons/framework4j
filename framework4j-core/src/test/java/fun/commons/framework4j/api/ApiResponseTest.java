...
class ApiResponseTest {
    ...
    void testFailWithCustomMessage() {
        ApiResponse<Void> response = ApiResponse.fail(ApiCode.RESOURCE_NOT_FOUND, "用户 ID 999 不存在");
    ...
    void testJsonSerializationSuccess() {
    ...
    void testJsonSerializationFail() {
        ApiResponse<Void> response = ApiResponse.fail(ApiCode.UNAUTHORIZED);
        // 使用 WriteNulls 特性确保null值被序列化
        String json = JSON.toJSONString(response, com.alibaba.fastjson2.JSONWriter.Feature.WriteNulls);
    ...
    void testApiCodeFromCode() {
        ...
        assertEquals(ApiCode.RESOURCE_NOT_FOUND, code);

        // 测试未找到的情况
        ApiCode unknownCode = ApiCode.fromCode(99999);
        assertEquals(ApiCode.SYSTEM_BUSY, unknownCode);
    }
