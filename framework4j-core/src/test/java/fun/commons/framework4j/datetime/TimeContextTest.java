...
class TimeContextTest {
    ...
    @Nested
    @DisplayName("与DynamicTimeFilter集成测试")
    class DynamicTimeFilterIntegrationTest {

        @Test
        @DisplayName("TimeContext设置应该影响DynamicTimeFilter的行为")
        void timeContextShouldAffectDynamicTimeFilterBehavior() {
            // Given
            DynamicTimeFilter filter = new DynamicTimeFilter();
            TestObject obj = new TestObject();
            obj.setTime(java.time.OffsetDateTime.now());

            // When - 不设置本地格式
            TimeContext.setUseLocal(false);
            Object result1 = filter.apply(obj, "time", obj.getTime());

            // When - 设置本地格式
            TimeContext.setUseLocal(true);
            Object result2 = filter.apply(obj, "time", obj.getTime());

            // Then
            assertThat(result1).isInstanceOf(java.time.OffsetDateTime.class); // 原始对象
            assertThat(result2).isInstanceOf(String.class); // 格式化后的字符串
