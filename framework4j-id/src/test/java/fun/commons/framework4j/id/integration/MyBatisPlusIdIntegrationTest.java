...
/**
 * MyBatis-Plus ID 生成器集成测试
 * <p>
 * 验证 {@code MpIdGenerator} 作为自定义 {@code IdentifierGenerator} 与 MyBatis-Plus 的集成：
 * <ul>
 *   <li>{@code @TableId(IdType.ASSIGN_ID)} 自动触发 MpIdGenerator</li>
 *   <li>批量插入 ID 唯一性</li>
 *   <li>生成的 ID 可被 {@link IdAnalysisTool} 反解析</li>
 *   <li>标准 CRUD（insert/select/update/delete）</li>
 * </ul>
 * <p>
 * 使用 H2 内存数据库（PostgreSQL 兼容模式），不依赖真实 PostgreSQL。
 * 如需真实 PostgreSQL 集成测试，参见 {@code framework4j-testkit}（第二期规划）。
 */
@SpringBootTest(classes = MyBatisPlusIdIntegrationTest.TestApplication.class)
@ActiveProfiles("mptest")
@DisplayName("MyBatis-Plus ID 生成器集成测试")
@TestMethodOrder(MethodOrderer.OrderAnnotation.class)
class MyBatisPlusIdIntegrationTest {
    ...
    @SpringBootApplication(exclude = {
            fun.commons.framework4j.web.config.WebAutoConfiguration.class
    })
    @org.mybatis.spring.annotation.MapperScan("fun.commons.framework4j.id.integration")
