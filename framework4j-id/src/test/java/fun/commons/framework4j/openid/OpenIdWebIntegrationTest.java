...
import fun.commons.framework4j.web.config.WebConfig;
...
class OpenIdWebIntegrationTest {
    ...
    void shouldDeserializePathVariable() throws Exception {
        ...
        mockMvc.perform(get("/fun/commons/framework4j/api/test/path/{id}", openId))
                .andDo(print())
    ...
    void shouldDeserializeRequestParam() throws Exception {
        ...
        mockMvc.perform(get("/fun/commons/framework4j/api/test/param").param("id", openId))
                .andDo(print())
                .andExpect(status().isOk())
                .andExpect(content().string("Received: " + originalId));
    }
    ...
    void shouldHandleRawNumberInput() throws Exception {
        ...
        mockMvc.perform(get("/fun/commons/framework4j/api/test/path/{id}", rawId))
                .andExpect(status().isOk())
    ...
    void shouldSerializeJsonWithObfuscation() throws Exception {
        ...
        mockMvc.perform(get("/fun/commons/framework4j/api/test/json").param("id", String.valueOf(id)))
                .andDo(print())
                .andExpect(status().isOk())
    ...
    void shouldSerializeListWithObfuscation() throws Exception {
        ...
        mockMvc.perform(get("/fun/commons/framework4j/api/test/list")
    ...
    void shouldGenerateCorrectOpenApiDocs() throws Exception {
        ...
        mockMvc.perform(get("/v3/api-docs"))
                ...
                .andExpect(jsonPath("$.paths['/fun/commons/framework4j/api/test/path/{id}'].get.parameters[0].name").value("id"))
                .andExpect(jsonPath("$.paths['/fun/commons/framework4j/api/test/path/{id}'].get.parameters[0].schema.type").value("string"))

                // 2. 验证入参: param?id=... 的 id 参数类型应为 string
                .andExpect(jsonPath("$.paths['/fun/commons/framework4j/api/test/param'].get.parameters[0].name").value("id"))
                .andExpect(jsonPath("$.paths['/fun/commons/framework4j/api/test/param'].get.parameters[0].schema.type").value("string"))
    ...
    void shouldFormatOffsetDateTimeToIso8601() throws Exception {
        MvcResult result = mockMvc.perform(get("/fun/commons/framework4j/api/test/datetime"))
                .andDo(print())
                .andExpect(status().isOk())
                .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
                .andReturn();
    ...
    void shouldFormatFixedOffsetDateTime() throws Exception {
        MvcResult result = mockMvc.perform(get("/fun/commons/framework4j/api/test/fixed-datetime"))
                .andDo(print())
                .andExpect(status().isOk())
                .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
                .andReturn();
    ...
    @RequestMapping("/fun/commons/framework4j/api/test")
